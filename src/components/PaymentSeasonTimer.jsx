...
const COLLAPSED_KEY = 'autocredito_timer_collapsed_v1';
...
export default function PaymentSeasonTimer() {
  ...
  return (
    <div className="glass-panel" style={{ margin: '0 12px 14px 12px', border: '1px solid rgba(255, 159, 28, 0.3)', overflow: 'hidden' }}>

      {/* ─── Barra de título siempre visible — clic para colapsar/expandir ─── */}
      <button
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ fontSize: '1.15rem' }}>{isExpired ? '✅' : '⏳'}</span>
          <div style={{ textAlign: 'left' }}>
            <span style={{ fontSize: '0.82rem', fontWeight: 700, color: timerMode === 'sorteo' ? 'var(--primary)' : '#00b4d8' }}>
              {timerMode === 'sorteo' ? '🏆 Cuenta Regresiva — Sorteo AutoCrédito' : `💳 Vencimiento ${DEADLINES[selectedPlan].label}`}
            </span>
            {!isCollapsed && (
              <span style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-dim)' }}>
                Toca para contraer el widget
              </span>
            )}
            {isCollapsed && (
      ...
      {!isCollapsed && (
        <div style={{ padding: '0 14px 14px 14px' }}>
          ...
          {/* Remaining content — everything that was inside the original container */}
          {/* Top Banner Row */}
